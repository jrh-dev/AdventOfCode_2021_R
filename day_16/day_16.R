...
for (ii in 1:195) {
  last_count <- length(count)
  last_stop <- length(stop_on)
  
  if (is.na(catch_pack_len[ii])) {
    count[last_count] <- count[last_count] + 1
    string <- paste0(string, catch_lit[ii])
  } else {
    stop_on <- c(stop_on, catch_pack_len[ii])
    pt <- as.character(catch_pack_type[ii])
    write <- switch(
      pt,
      "0" = "sum(",
      "1" = "prod(",
    ...
    string <- paste0(string, write)
    count <- c(count, 0)
  }
  
  if (count[last_count] == stop_on[last_stop]) {
    last_count <- length(count)
    last_stop <- length(stop_on)
    tmp_ls <- length(stop_on) - 1
    while (count[last_count] == stop_on[last_stop]) {
      count[tmp_ls] <- count[tmp_ls] + 1
      count <- count[1:last_count - 1]
    stop_on <- stop_on[1:last_stop - 1]
    string <- paste0(string, ")")
    last_count <- length(count)
    last_stop <- length(stop_on)
    }
    string <- paste0(string, ",")
  } else if (is.na(catch_pack_len[ii])) {
    string <- paste0(string, ",")
  }
